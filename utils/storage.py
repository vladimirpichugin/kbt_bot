...
class Storage:
    ...
    def save_client(self, user: User, client: Client) -> bool:
        if not client.changed:
            logger.debug(f'User <{user.id}:{user.username}> already saved, data not changed.')
            return True

        save = self.save_data(self.clients, user.id, client)

        if save:
            logger.debug(f'User <{user.id}:{user.username}> saved, result: {save}')
            return True

        logger.error(f'User <{user.id}:{user.username}> not saved, result: {save}')
